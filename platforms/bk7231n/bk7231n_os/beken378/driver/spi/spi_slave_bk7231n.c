...
{
	UINT32 param;
	os_printf("spi slave mode:%d, rate;%d\r\n", mode, rate);
	bk_spi_slave_configure(rate, mode);
	...
	os_printf("slave [CTRL]:0x%08x \n", REG_READ(SPI_CTRL));
	os_printf("slave [CONFIG]:0x%08x \n", REG_READ(SPI_CONFIG));
}
