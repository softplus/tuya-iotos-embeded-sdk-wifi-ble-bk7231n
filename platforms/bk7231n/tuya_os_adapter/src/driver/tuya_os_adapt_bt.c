...
static BOOL_T ble_scan_enable = FALSE;
                ...
                case TY_BT_VER11_PROTOCOL_TYPE_SER_DATA:
                    {
                        ...
                        if(ble_scan_info->scan_adv_cb){
                            ...
                            mac[5] = recv_adv->adv_addr[0];
                            mac[4] = recv_adv->adv_addr[1];
                            mac[3] = recv_adv->adv_addr[2];
                            mac[2] = recv_adv->adv_addr[3];
                            mac[1] = recv_adv->adv_addr[4];
                            mac[0] = recv_adv->adv_addr[5];
...
int tuya_os_adapt_bt_port_init(ty_bt_param_t *p)
{
    ...
    LOG_DEBUG("bt_port_init\r\n");

    if((p != NULL)&&(p->cb != NULL)) {
        ty_bt_msg_cb = p->cb;
    } else {
        LOG_ERR("bt_port_init, p is null\r\n");
    }
...
int tuya_os_adapt_bt_port_deinit(void)
{
    ...
    //LOG_DEBUG("bt_port_deinit stat:%d\r\n", app_status);
...
OPERATE_RET tuya_os_adapt_bt_start_scan(void)
{
    if(ble_scan_info && (TY_BT_SCAN_BY_ADV == ble_scan_info->scan_type)) {
        if (ERR_SUCCESS == appm_start_scaning()) {
            LOG_DEBUG("start scan\r\n");
            ble_scan_enable = TRUE;
            return OPRT_OS_ADAPTER_OK;
        }
    }
...
{
    if(ble_scan_info &&  (TY_BT_SCAN_BY_ADV == ble_scan_info->scan_type)){
        if (ERR_SUCCESS == appm_stop_scaning()) {
            LOG_DEBUG("stop scan\r\n");
            ble_scan_enable = FALSE;
            return OPRT_OS_ADAPTER_OK;
        }
    }
